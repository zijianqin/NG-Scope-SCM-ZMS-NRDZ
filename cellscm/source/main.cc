...
int main(int argc, char** argv)
{
    ...
    if(!prog_args.input) {
        ...
        if (cell.nof_prb == 100) {
            cells.cell_cfg.bw = 20;
        } else if (cell.nof_prb == 75) {
            cells.cell_cfg.bw = 15;
        } else if (cell.nof_prb == 50) {
            cells.cell_cfg.bw = 10;
        } else if (cell.nof_prb == 25) {
            cells.cell_cfg.bw = 5;
        } else if (cell.nof_prb == 15) {
            cells.cell_cfg.bw = 3;
        } else if (cell.nof_prb == 6) {
            cells.cell_cfg.bw = 1.4;
        } else {
                        ...
                        for (uint32_t tb = 0; tb < SRSRAN_MAX_CODEWORDS; tb++) {
                            if (pdsch_cfg.grant.tb[tb].enabled) {
                                if (acks[tb]) { 
                                    ...
                                    asn1::cbit_ref dlsch_bref(payload, pdsch_cfg.grant.tb[0].tbs / 8);
                                    asn1::json_writer js_sib1;
                                    asn1::json_writer js_sib2;
                                    asn1::json_writer js_plmn_id;
                                    ...
                                    if (err == asn1::SRSASN_SUCCESS) {
                                        if ((dlsch.msg.c1().type() == asn1::rrc::bcch_dl_sch_msg_type_c::c1_c_::types::sib_type1)) {
                                            ...
                                            cells.cell_cfg.sib1 = sib1;
                                            printf("mcc:%d%d%d\n", sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mcc[0], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mcc[1], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mcc[2]);
                                            printf("mnc:%d%d%d\n", sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mnc[0], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mnc[1], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mnc[2]);
                                            ...
                                            if (file == NULL) {
                                                ...
                                                fprintf(file, "cell.mcc:%d%d%d\n", sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mcc[0], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mcc[1], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mcc[2]);
                                                fprintf(file, "cell.mnc:%d%d%d\n", sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mnc[0], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mnc[1], sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mnc[2]);
                                                fprintf(file, "cell.tac=%ld\n", sib1.cell_access_related_info.tac.to_number());
                                                fprintf(file, "cell.id=%ld\n", sib1.cell_access_related_info.cell_id.to_number());
                                            }
                                            fclose(file);
                                            }
                                            ...
                                            for (uint32_t i = 0; i < sib_list.size(); i++) {
                                                if ((sib_list[i].type().value == asn1::rrc::sib_info_item_c::types::sib2) && (have_sib2 == 0)) {
                                                    ...
                                                    cells.cell_cfg.sib2 = sib_list[i].sib2();
                                                    FILE *file = fopen("celloutput.txt", "a");
                                                    if (file == NULL) {
                                                        perror("Error openning the file.\n");
                                                    } else {
                                                        fprintf(file, "cell.pdsch_reference_signal_power=%ddBm\n", cells.cell_cfg.sib2.rr_cfg_common.pdsch_cfg_common.ref_sig_pwr);
                                                    }
                                                    fclose(file);
                                                }
                                            }
